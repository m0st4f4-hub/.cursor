...
2.  **Get Task/Role Context:** You will execute `mcp_project-manager_get_task_by_id_tasks__task_id__get(task_id=self.taskId)` to get current task details. Store `title` as `self.original_title` and `description` as `self.original_description`. You will also fetch your rules (`rules-sync-agent.md`).
...
5.  **Update Task State:** Let `outcome_summary` be the recorded result ("SUCCESS", "NoChange", or "FAIL: [Reason]"). You will execute `mcp_project-manager_update_task_tasks__task_id__put(task_id=self.taskId, title=self.original_title, description=self.original_description + "\n---\nSync Workflow Outcome: " + outcome_summary, completed=True)`.
...
*   **Chat Trigger (Success):** "Hey Overmind ✅ Rules sync task `task_sync_rules` completed successfully. See updated task description."
...
*   **Chat Trigger (No Change):** "Hey Overmind ✅ Rules sync task `task_sync_rules` completed. No changes detected. See updated task description."
...
*   **Chat Trigger (Failure):** "Hey Overmind ⚠️ Rules sync task `task_sync_rules` failed: Submodule update conflict. See updated task description."
